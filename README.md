...
응답을 저장할 빈 Google 스프레드시트를 만들거나 기존 시트를 준비합니다. 시트 ID는 URL 의 `/d/` 와 `/edit` 사이 문자열입니다.

> 보안상 시트 ID는 코드에 하드코딩하지 않고 Apps Script 의 **스크립트 속성**에 보관합니다(아래 2단계).
> 시트 공유 권한은 **"제한됨"** (본인만)으로 두는 것을 권장합니다. Apps Script 가 "나로 실행"으로 배포되므로 응답자는 시트 권한이 없어도 정상 제출됩니다.
...
3. 저장 (Ctrl/Cmd + S)
4. 좌측 **톱니바퀴(프로젝트 설정)** → 하단 **"스크립트 속성"** → **속성 추가**
   - 속성: `SPREADSHEET_ID`
   - 값: 1단계의 시트 ID
   - 저장
5. 좌측 함수 선택 드롭다운에서 `setupHeaders` 선택 → **실행** 클릭
   - 권한 요청이 뜨면 본인 Google 계정으로 승인
   - 완료 후 시트에 `응답` 탭이 만들어지고 1행(변수명)·2행(라벨)이 자동 생성됨
6. 우상단 **배포 → 새 배포 → 톱니바퀴 → 웹앱** 선택
   - 설명: 자유롭게
   - **다음 사용자 인증으로 실행: 나**
